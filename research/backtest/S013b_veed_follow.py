"""
research/backtest/S013b_veed_follow.py — S-013b VEED Follow Kernel (BTCUSDT 5m)

Per factory/preregistrations/S013b_veed_follow.md (owner-approved, Phase 2B-2).

FOLLOWS VEED v1.1 events: sell-side event → SHORT, buy-side event → LONG.
...
Mirror of S-013 (fade): opposite direction, same data/detector/windows/rails.
...
EXPERIMENT_ID = "S-013b"
STRATEGY_FAMILY = "VEED-FOLLOW"
PARAM_DESC = "Follow VEED v1.1 events (sell→SHORT, buy→LONG), stop at event extreme, 1.5R"
...
def detect_events(bars):
    """Re-implement VEED v1.1 detection inline (deterministic)."""
    n = len(bars)
    events = []
        ...
        if not position_open and ei < len(events):
            ...
            if ev_idx == i:
                if i + 1 < n:
                    ...
                    # FOLLOW (opposite of S-013 fade):
                    #   sell event → SHORT (follow the dump)
                    #   buy event  → LONG (follow the rally)
                    if side == "sell":
                        stop_price = highs[i]
