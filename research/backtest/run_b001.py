"""
research/backtest/run_b001.py — Phase 3 B-001 Structural Batch Runner

Executes the 5 structural kernels (S-001, S-002, S-004, S-005, S-008) through
the IS -> OOS ladder and generates the official B-001 Batch Report.
...
One shot per window per kernel. Cool-off waived per charter amendment.
...
# Cool-off (waived per charter amendment for SHA-locked preregistered kernels)
COOL_OFF_HOURS = 0
...
# Structural kernels (in order)
STRUCTURAL_KERNELS = [
    ("S-001", "S001_liquidity_sweep", "Liquidity Sweep Fade"),
    ("S-002", "S002_choch", "Change-of-Character Reversal"),
    ("S-004", "S004_goldzone", "Gold-Zone Retracement Fade"),
    ("S-005", "S005_invalidation_flip", "Invalidation Flip"),
    ("S-008", "S008_volcompression", "Volatility Compression Breakout"),
]
...
def run_batch(phase_filter=None):
    """Run the full structural batch and generate the report."""
    print("=" * 78)
    print(f"PHASE 3 STRUCTURAL BATCH — {BATCH_ID}")
    print("=" * 78)
    ...
    for kernel_id, kernel_file, description in STRUCTURAL_KERNELS:
        ...
        kernel_module = load_kernel(kernel_file)
        ...
        if not is_completed and phase_filter != PHASE_OOS:
            ...
            # OOS-only mode: IS already completed, re-run to get metrics for WR drop check
            try:
                success, is_metrics, is_verdict, is_ds_hash = evaluate_phase(
                    kernel_module, dataset_path, raw_dir, PHASE_IS, gate1_log
                )
            ...
            # IS completed, not OOS-only: re-run to get verdict
            try:
                success, is_metrics, is_verdict, is_ds_hash = evaluate_phase(
                    kernel_module, dataset_path, raw_dir, PHASE_IS, gate1_log
                )
    ...
    # Calculate metrics
    n_requested = len(STRUCTURAL_KERNELS)
    n_effective = n_requested
    ...
    print("\n" + "=" * 78)
    print("PHASE 3 BATCH REPORT")
    print("=" * 78)
    print(f"BATCH: {BATCH_ID} (Structural)")
    ...
    print(f"NULL PASS RATE: {null_pass_rate:.0%}")
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Phase 3 Structural Batch Runner")
