"""
h004_tight_stop.py — H-004: H-001 with stop tightened to 0.75xSD.

BACKLOG ITEM — scaffold only, no backtest yet.
Purpose: map the stop-dial frontier (auditor prediction: PF drops below 1.0).

Parent: H-001 (provenance: OWNER)
DNA: same mechanism as H-001, different parameter_hash (stop_mult=0.75)
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(os.path.join("logs", "h004.log")),
        logging.StreamHandler(),
    ],
)
logger = logging.getLogger("h004")

# ---------------------------------------------------------------------------
# PRE-COMMITTED PARAMETERS (H-004, preregistered)
# ---------------------------------------------------------------------------
EXPERIMENT_ID = "H-004"
PROCESS_VERSION = "v1"
STRATEGY_FAMILY = "MR"
PARAM_DESC = "Z>2 SMA20 1SD-band reversion, Asian 22:00-07:00, maxhold 8, SL 0.75xSD (tight stop frontier)"
...
SL_SD_MULT = 0.75  # TIGHTENED from 1.5 (H-001) to 0.75
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="H-004 tight stop backtest")
    ...
    print("=" * 78)
    print(f"{EXPERIMENT_ID} | Tight stop EURUSD H1 | {PARAM_DESC}")
    ...
    output_path = os.path.join("research", f"trade_log_{args.phase}_h004.csv")
