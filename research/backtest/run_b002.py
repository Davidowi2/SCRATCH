"""
research/backtest/run_b002.py — Phase 4 B-002 S-010 Single Kernel Runner.

Executes S-010 through the IS ladder and generates the official B-002 report.
...
# Single kernel for B-002
KERNELS = [
    ("S-010", "S010_banklevel_fx", "Bank-Level FX Arm: Asian range breakout"),
]
...
def run_batch(phase_filter=None):
    """Run the batch and generate the report."""
    print("=" * 78)
    print(f"PHASE 4 SINGLE-KERNEL BATCH — {BATCH_ID}")
    print("=" * 78)
    ...
    for kernel_id, kernel_file, description in KERNELS:
        ...
        # If IS already completed, skip
        if is_completed and phase_filter != PHASE_OOS:
        ...
        is_v_hash = make_verdict_hash(kernel_id, BATCH_ID, PHASE_IS, ds_hash, is_metrics, is_verdict)
        with open(ledger_path, "a", newline="") as f:
            writer = csv.writer(f)
            writer.writerow([BATCH_ID, kernel_id, PHASE_IS, datetime.utcnow().isoformat(), ds_hash, is_v_hash, "COMPLETED"])
        ...
        if phase_filter == PHASE_IS:
            results.append({
                "kernel_id": kernel_id, "description": description,
                "is_verdict": is_verdict[0], "is_note": "IS passed",
                "oos_verdict": None, "oos_note": None, "dataset_hash": ds_hash,
                "verdict_hash": is_v_hash,
            })
            continue
        ...
        results.append({
            ...
            "dataset_hash": ds_hash, "verdict_hash": is_v_hash,
    ...
    print("\n" + "=" * 78)
    print("PHASE 4 BATCH REPORT")
    print("=" * 78)
    print(f"BATCH: {BATCH_ID} (Single-Kernel)")
    ...
    print(f"NULL PASS RATE (calibrated 2026-09-13): {null_pass_rate:.0%}")
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Phase 4 B-002 Single-Kernel Runner")
